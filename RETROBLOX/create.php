...
try {
    $pdo = new PDO("mysql:host=$server;dbname=$database", $db_user, $db_password);
    $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);

    // Verificar se o usuário já tem um jogo criado
    $stmt = $pdo->prepare("SELECT * FROM game WHERE creator_name = :creator_name");
    $stmt->execute(['creator_name' => $username]);
    $existing_game = $stmt->fetch();

    // Se o formulário foi enviado
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        // Verificar se os campos estão vazios
        if (empty($_POST['game_name']) || empty($_POST['game_description']) || empty($_POST['game_pip']) || !isset($_POST['public'])) {
            echo "<p style='color:red;'>All fields must be filled in!</p>";
        } else {
            if ($existing_game) {
                echo "<p style='color:red;'>You cannot create more than one game.</p>";
            } else {
                // Coletar dados do formulário
                $game_name = $_POST['game_name'];
                $game_description = $_POST['game_description'];
                $game_pip = $_POST['game_pip'];  // Aqui, estamos pegando o valor de game_pip como 'ip'
                $public = $_POST['public'];      // Pega o valor de "public" da escolha do usuário

                // Inserir novo jogo na tabela
                $stmt = $pdo->prepare("INSERT INTO game (creator_name, creator_id, name, description, public, ip) 
                                       VALUES (:creator_name, :creator_id, :game_name, :game_description, :public, :game_pip)");
                $stmt->execute([
                ...
                echo "<p style='color:green;'>Game created successfully!</p>";
            }
        }
    }
...
.items-container p {
    ...
    color: #555555;
      ...
      .radio-group {
        ...
        /* Ajustando a disposição dos botões de rádio */
        .radio-group input {
...
<br>
   <div class="gamecreation">
        ...
        <form method="POST">
            ...
            <input type="text" name="game_pip" placeholder="IP Game" required>

            <!-- Radio para a escolha de Publicidade -->
            <div class="radio-group">
                <label>Public Game?</label><br>
                <input type="radio" name="public" value="yes" required>Yes:
                <input type="radio" name="public" value="no" required>No:
            </div>
