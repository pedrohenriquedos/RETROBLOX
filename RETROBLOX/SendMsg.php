...
$servidor = 'sql103.ezyro.com';  // Novo nome para a variável de host
$data = 'ezyro_38183570_users';    // Novo nome para o nome do banco de dados
$db = 'ezyro_38183570';           // Novo nome para o usuário
$db_pass = '';        // Novo nome para a senha
...
// Conexão com o banco de dados usando as novas credenciais
$servername = 'sql103.ezyro.com';  // Novo nome para a variável de host
$username = 'ezyro_38183570';      // Novo nome para o nome de usuário
$password = '';         // Novo nome para a senha do banco de dados
$dbname = 'ezyro_38183570_users';  // Novo nome para o nome do banco de dados
...
// SQL para pegar as mensagens onde user_recive é o mesmo que o usuário da sessão
$sql = "SELECT * FROM mensagens WHERE user_recive = '$usernameee'";
$result = $conn->query($sql);
?>
<?php
session_start();
...
if (isset($_GET['user'])) {
    $receiver = $_GET['user'];
    if (strtolower($receiver) == strtolower($usernameee)) {
...
// Definir o cooldown (em segundos)
$cooldownTime = 60; // 60 segundos

// Verificar se a sessão 'last_message_time' existe
if (isset($_SESSION['last_message_time'])) {
    $timeDiff = time() - $_SESSION['last_message_time'];
} else {
    $timeDiff = $cooldownTime; // Se a sessão não existir, o usuário pode enviar a mensagem
}

// Se o tempo de cooldown passou, o usuário pode enviar a mensagem
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if ($timeDiff >= $cooldownTime) {
        ...
        // Recriar a conexão para enviar a mensagem
        $conn = new mysqli($servername, $username, $password, $dbname);
        ...
        $stmt = $conn->prepare("INSERT INTO mensagens (user_send, user_recive, message_text, title, id_send) VALUES (?, ?, ?, ?, ?)");
        $stmt->bind_param("ssssi", $sender, $receiver, $message_text, $title, $user_id);

        if ($stmt->execute()) {
            // Atualiza o tempo da última mensagem
            $_SESSION['last_message_time'] = time();
            echo "<div class='message-status'>Message successfully sent to $receiver!</div>";
        } else {
            echo "<div class='message-status error'>Error</div>";
        }

        $conn->close();
    } else {
        // Exibir o tempo restante do cooldown
        $remainingTime = $cooldownTime - $timeDiff;
        echo "<div class='message-status error'>You must wait $remainingTime seconds before sending another message.</div>";
    }
}
?>
    ...
    <div class="container">
        ...
        <div class="banner">
         ...
         <div class="welcome-text">
            Logged:<?php echo htmlspecialchars($usernameee); ?> 
...
<br>
  <div class="me-container">
        <h1>Send Message to <?php echo htmlspecialchars($receiver); ?></h1>
        <form action="SendMsg.php?user=<?php echo urlencode($receiver); ?>" method="POST">
    ...
    if (isset($_SESSION['last_message_time'])) {
        ...
        if ($timeDiff < $cooldownTime) {
            $remainingTime = $cooldownTime - $timeDiff;
            echo "<div class='cooldown-timer'>You can send another message in $remainingTime seconds.</div>";
        }
    }
    ?>
    </div>