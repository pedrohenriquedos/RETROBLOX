...
// Pegando o 'id' do fórum da URL
$forum_id = isset($_GET['id']) ? (int)$_GET['id'] : 0; // Se o 'id' não for passado, assume 0

// Processando o envio do post (resposta)
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $response_text = trim($_POST['response_text']); // O conteúdo da resposta

    // Pegando os dados do usuário da sessão

    // Inserindo a resposta na tabela 'forum_replies'
    $sql_insert = "INSERT INTO forum_replies (forum_id, user_id, response_text, user_reply, avatar) 
                   VALUES (?, ?, ?, ?, ?)";
    ...
    // Vinculando os parâmetros à declaração preparada
    $stmt_insert->bind_param('iisss', $forum_id, $id, $response_text, $username, $avatar_img);

    if ($stmt_insert->execute()) {
        // Redireciona para a página do fórum após a resposta ser enviada
        header("Location: ShowForum.php?id=" . $forum_id);
        exit();
    } else {
        echo "Erro: " . $conn_forum->error;
    }
}
...
?>
    ...
    <form method="POST">
        ...
        <textarea name="response_text" id="response_text" rows="5" required></textarea>
